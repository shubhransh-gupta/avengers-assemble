...
export class GeminiProvider extends BaseProvider {
  ...
  async execute(prompt: string, options?: ProviderExecutionOptions): Promise<ProviderResponse> {
    ...
    const candidateModels = options?.model
      ? [options.model, 'gemini-3.5-flash-lite', 'gemini-3.1-flash-lite', 'gemini-flash-latest']
      : ['gemini-3.5-flash-lite', 'gemini-3.1-flash-lite', 'gemini-flash-latest', 'gemini-3.7-flash'];
    ...
    throw new Error(`[GeminiProvider] All candidate models failed. Last error: ${lastError?.message}`);
  }
}
